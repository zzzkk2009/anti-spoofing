...
def gen_data(anno_dirname, size=20, base_num = 1, prob_thresh = 0.3, thread_num = 4 ):
    anno_file = "%s/prepare_data/%s/anno.txt"%(config.root, anno_dirname)
    prob_file = "%s/prepare_data/%s/prob.txt"%(config.root, anno_dirname)
...
def parse_args():
    ...
    parser.add_argument('--anno_dirname', dest='anno_dirname', help='anno dirname', type=str)
...
if __name__ == '__main__':
    ...
    print(args)
    gen_data(args.anno_dirname, int(args.size), int(args.base_num), float(args.prob_thresh), int(args.thread_num))