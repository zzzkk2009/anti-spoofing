...
cv::Mat ncnn2cv(ncnn::Mat img)
{
	unsigned char* pix = new unsigned char[img.h * img.w * 3];
	img.to_pixels(pix, ncnn::Mat::PIXEL_BGR);
	cv::Mat cv_img(img.h, img.w, CV_8UC3);
	for (int i = 0; i < cv_img.rows; i++)
	{
...
int main(int argc, char* argv[])
{
	...
	mtdet_ncnn::MtcnnDetector_ncnn detector("./model/insightface/insightface_ncnn");

    double start = (double)getTickCount();
    vector<FaceInfo_ncnn> results1 = detector.Detect(ncnn_img1);
    cout << "Detection Time: " << (getTickCount() - start) / getTickFrequency() << "s" << std::endl;

    start = (double)getTickCount();
    vector<FaceInfo_ncnn> results2 = detector.Detect(ncnn_img2);
    cout << "Detection Time: " << (getTickCount() - start) / getTickFrequency() << "s" << std::endl;
    ...
    Arcface arc("./model/insightface/insightface_ncnn");
