...
int main(int argc, char **argv)
{
	...
	while (true)
	{
		try
		{
			...
			int iResponse = detector.detectSpoofing2(rgb_cameraFrames, ir_cameraFrames);
			...
			/*if (detector.getStartSampling())
			{
				Mat motion2color = detector.getMotion2color();
				if (detector.getSamplePositiveData())
				{
					util::gatherDataSet(motion2color, "./data/48x64/1/");
				}
				else
				{
					util::gatherDataSet(motion2color, "./data/48x64/0/");
				}
			}*/
