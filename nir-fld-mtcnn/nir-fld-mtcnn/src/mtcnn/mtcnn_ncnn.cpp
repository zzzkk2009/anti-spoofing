...
namespace mtdet_ncnn {
	MtcnnDetector_ncnn::MtcnnDetector_ncnn(string model_folder)
	{
	...
	MtcnnDetector_ncnn::~MtcnnDetector_ncnn()
	{
		this->Pnet.clear();
		this->Rnet.clear();
		this->Onet.clear();
	...
	vector<FaceInfo_ncnn> MtcnnDetector_ncnn::Detect(ncnn::Mat img)
	{
		int img_w = img.w;
		int img_h = img.h;

		vector<FaceInfo_ncnn> pnet_results = Pnet_Detect(img);
		doNms(pnet_results, 0.7, "union");
		refine(pnet_results, img_h, img_w, true);

		vector<FaceInfo_ncnn> rnet_results = Rnet_Detect(img, pnet_results);
		doNms(rnet_results, 0.7, "union");
		refine(rnet_results, img_h, img_w, true);

		vector<FaceInfo_ncnn> onet_results = Onet_Detect(img, rnet_results);
	...
	vector<FaceInfo_ncnn> MtcnnDetector_ncnn::Pnet_Detect(ncnn::Mat img)
	{
		vector<FaceInfo_ncnn> results;
		int img_w = img.w;
		int img_h = img.h;
		...
		for (auto it = scales.begin(); it != scales.end(); it++)
		{
			...
			vector<FaceInfo_ncnn> bboxs = generateBbox(score, location, *it, this->threshold[0]);
	...
	vector<FaceInfo_ncnn> MtcnnDetector_ncnn::Rnet_Detect(ncnn::Mat img, vector<FaceInfo_ncnn> bboxs)
	{
		vector<FaceInfo_ncnn> results;
	...
	vector<FaceInfo_ncnn> MtcnnDetector_ncnn::Onet_Detect(ncnn::Mat img, vector<FaceInfo_ncnn> bboxs)
	{
		vector<FaceInfo_ncnn> results;
	...
	void MtcnnDetector_ncnn::Lnet_Detect(ncnn::Mat img, vector<FaceInfo_ncnn> &bboxes)
	{
		int img_w = img.w;
		int img_h = img.h;
	...
	vector<FaceInfo_ncnn> MtcnnDetector_ncnn::generateBbox(ncnn::Mat score, ncnn::Mat loc, float scale, float thresh)
	{
		int stride = 2;
		int cellsize = 12;
	...
	bool cmpScore(FaceInfo_ncnn x, FaceInfo_ncnn y)
	{
		if (x.score > y.score)
			return true;
		else
	...
	float calcIOU(FaceInfo_ncnn box1, FaceInfo_ncnn box2, string mode)
	{
	...
	void MtcnnDetector_ncnn::doNms(vector<FaceInfo_ncnn> &bboxs, float nms_thresh, string mode)
	{
		if (bboxs.empty())
			return;
		...
		/*for(vector<FaceInfo_ncnn>::iterator it = bboxs.begin(); it != bboxs.end();){*/
		for (auto it = bboxs.begin(); it != bboxs.end();) {
			if ((*it).score == 0) {
				//bboxs.erase(it); // vector iterators incompatible
				it = bboxs.erase(it);
			}
			else {
				it++;
	...
	void MtcnnDetector_ncnn::refine(vector<FaceInfo_ncnn> &bboxs, int height, int width, bool flag)
	{
		if (bboxs.empty())
			return;
		for (auto it = bboxs.begin(); it != bboxs.end(); it++)
