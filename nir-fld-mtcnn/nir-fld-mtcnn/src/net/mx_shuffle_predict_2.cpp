...
mx_uint predict_img_height, predict_img_width = 0;
...
namespace shuffle {
	...
	int initPredictor(PredictorHandle &predictor, string symbol_file, string params_file, mx_uint _predict_img_height, mx_uint _predict_img_width)
	{
		...
		/*string prefix = "./src/net/model/mx-flow-shuffle/checkpoint";
		string symbol_file = prefix + "-symbol.json";
		string params_file = prefix + "-0060.params";*/
		int dev_type = 1; // 1:CPU, 2:GPU
		int dev_id = 0;
		mx_uint num_input = 1;
		const char* input_keys[] = { "data" };
		const mx_uint input_shape_indptr[] = { 0,4 };
		//const mx_uint input_shape_data[] = { 1,3,kIMSIZE,kIMSIZE }; // batch_size, channel, height, width

		predict_img_height = _predict_img_height;
		predict_img_width = _predict_img_width;
		const mx_uint input_shape_data[] = { 1,3,predict_img_height,predict_img_width }; // batch_size, channel, height, width
	...
	int predict(PredictorHandle predictor, Mat& img)
	{
		...
		cv::Size input_geometry = cv::Size(predict_img_width, predict_img_height);

		// Read and normalize image
		Mat netImage = preprocess(img, num_channels, input_geometry);

		//-- Set Input Image
		//MXPredSetInput(predictor, "data", (float *)netImage.data, kIMSIZE*kIMSIZE*num_channels);
		MXPredSetInput(predictor, "data", (float *)netImage.data, predict_img_height*predict_img_width*num_channels);
	...
	string getSynsetName(vector<string>& synsets, int label)
	{
		...
		if (synsets.size() > 0)
		{
			label_synset_str = synsets[label];
		}
		//cout << indmax << synsets[indmax] << endl;
		return label_synset_str;
	}
}
...
//
//		// Load synsets
//		vector<string> synsets = loadSynsets("./src/net/model/mx-flow-shuffle/synset.txt");
//
//		// Print max index
